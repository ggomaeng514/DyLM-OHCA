...
num_cpu   = 96
p = psutil.Process()
p.cpu_affinity( [int(num_cpu*cpu_set + i) for i in range(0, num_cpu)] )
...
def make_label(root_path, df, mode, start_sentence=0,window_size=0, max_sentence=85):
    ...
    df_dict = {'id':[], 's_id':[], 'json_file_path':[], 'wav_original_file_path':[] , 'endAt':[], 'label':[], 'label1':[], 'label2':[], 'text':[]}
            ...
            with open(file_path, 'r') as f:
                ...
                if isinstance(data['symptom'],list) and 'symptom' in data.keys():
                    ...
                    for i, each in enumerate(utterances):
                        ...
                        df_dict['id'].append(data['_id'])
                        df_dict['s_id'].append(data['_id']+f'_{i}') # for segment_id
                        df_dict['label'].append(label)
                        df_dict['label1'].append(label1)
                        df_dict['label2'].append(label2)
...
# train.to_csv('toy_data_json_audio_data_decoder_time_arrest_stack.csv', index=False)
...
# valid.to_csv('valid_json_audio_data_decoder_time_arrest_stack.csv', index=False)
...
# test.to_csv('test_json_audio_data_decoder_time_arrest_stack.csv', index=False)


##################### 90 seconds #####################
print('###########make_toy_data_label###########')
train=make_label(root_path='/storage/projects/NIA/datasets/cau/',df=train_df.iloc[:1000], mode='train',start_sentence=0,window_size=0, max_sentence=117)
train.to_csv('toy_data_json_audio_data_decoder_time_arrest_stack_90s.csv', index=False)
...
print('###########make_valid_label###########')
valid=make_label(root_path='/storage/projects/NIA/datasets/cau/',df=valid_df, mode='test',start_sentence=0,window_size=0, max_sentence=117)
valid.to_csv('valid_json_audio_data_decoder_time_arrest_stack_90s.csv', index=False)


print('###########make_test_label###########')

test=make_label(root_path='/storage/projects/NIA/datasets/cau/',df=test_df,mode='test',start_sentence=0,window_size=0, max_sentence=117)
test.to_csv('test_json_audio_data_decoder_time_arrest_stack_90s.csv', index=False)

# ##################### 60 seconds #####################
# print('###########make_toy_data_label###########')
# train=make_label(root_path='/storage/projects/NIA/datasets/cau/',df=train_df.iloc[:1000], mode='train',start_sentence=0,window_size=0, max_sentence=117)
# train.to_csv('toy_data_json_audio_data_decoder_time_arrest_stack_60s.csv', index=False)
...
# print('###########make_valid_label###########')
# valid=make_label(root_path='/storage/projects/NIA/datasets/cau/',df=valid_df, mode='test',start_sentence=0,window_size=0, max_sentence=117)
# valid.to_csv('valid_json_audio_data_decoder_time_arrest_stack_60s.csv', index=False)


# print('###########make_test_label###########')

# test=make_label(root_path='/storage/projects/NIA/datasets/cau/',df=test_df,mode='test',start_sentence=0,window_size=0, max_sentence=117)
# test.to_csv('test_json_audio_data_decoder_time_arrest_stack_60s.csv', index=False)
...
print(
# (sum(train['label']==True)/len(train),
...
print(
# (sum(train['label2']==True)/len(train),
