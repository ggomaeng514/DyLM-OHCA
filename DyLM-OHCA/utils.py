...
def calc_metric(predicted_probas, labels, round_sep=4):
    result = {
        'THRESHOLD' : [],
    ...
    for threshold in np.linspace(0, 1, 512):
        result['THRESHOLD'].append(round(threshold, round_sep))
        
        predicted_label = np.where(predicted_probas >= threshold , 1, 0)
        
        # result['ACC'].append(round(accuracy_score(labels, np.where(predicted_probas >= 0.5 , 1, 0)), round_sep))
        result['TH_ACC'].append(round(accuracy_score(labels, predicted_label), round_sep))
        result['AUROC'].append(round(roc_auc_score(labels, predicted_probas), round_sep))
        result['AUPRC'].append(round(average_precision_score(labels, predicted_probas), round_sep))
        result['RECALL'].append(round(recall_score(labels, predicted_label), round_sep))
        result['PRECISION'].append(round(precision_score(labels, predicted_label), round_sep))
        result['SPECIFICITY'].append(round(specificity_score(labels, predicted_label), round_sep))
        result['F1'].append(round(f1_score(labels, predicted_label), round_sep))
        result['BRIER'].append(round(brier_score_loss(labels, predicted_probas), round_sep))
